...
- Describe what people do with the allocation of resources (empirics)

- Develop stories about why they behave that way (theory)

Both of these require math:

- why statistical methods applied to data produce meaningful results (empirics)
...
Math studies what follows logically from precise assumptions.
...
- Things being random: what things? If one thing happened, can another thing happen at the same time? If one thing happened, does that make another thing more likely?
- Countries developing: what resources do they use? How are they combined? why do different countries get drastically different results from similar resources?
...
- minimum 10%
- higher rates for countries with a large trade deficit in goods, services trade ignored
- suggested to abolish income tax, use tariff income instead
...
The fundamental object of mathematics is a *set*, containing *elements*.

- Set of numbers $\{1,2,3\}$.
- Set of students $\{\text{John},\text{Megan},\text{Dylan},\text{Susie}...\}$.
- Set of students in this classroom.
- Set of students who are supposed in this classroom.
...
If there are elements in $A$ that are not in subset $B$, $B$ is a *proper subset* of $A$: $B \subset A$, and $A$ is a *proper superset* of $B$.
...
- set of affordable shopping baskets (the budget set)

When choosing where to study, pupils choose

- the set of university to apply - rules restrict it
- school to attend

When deciding on a stock/bond portfolio
...
- A *complement* of $A$ denoted as $\neg A= \{a: a \not \in A\}$ - need to know *what else* - all sets need to have a *space* $S$
- *union* of $A$ and $B$ denoted as $A \cup B=\{a \in A: a \in B\}$. 
- *intersection* of $A$ and $B$ denoted as $A \cap B= \{a\in A \text{ or } a \in B\}$. 
...
Prove: $A \cap B \subseteq A \cup B$. When can we claim that $A \cup B \subseteq A \cap B$?
...
\begin{tikzpicture}
\draw (-1,-1.4) rectangle (2,0.9) node[below left] {$S$};
 \draw[fill=red!70!black, draw=black, fill opacity=0.3] (0,0) circle (0.6cm) node[yshift=-1cm, color=red!40!white, fill opacity=1] {$A$};
...
*Space* $S$ is important!

## Exercises for Sets

Consider a standard 52-cardsdeck. A poker hand is 5 cards drawn from this deck at random. Verify claims:

- The set of poker hands featuring three of a kind is a subset of the set of poker hands featuring a pair.

- The set of poker hands featuring two pairs is a subset of the set of poker hands featuring a pair.

- The set of poker hands featuring a straight (cards in sequence of rank ignoring suits) and the set of poker hands featuring a pair do not overlap.
...
- The set of poker hands containing a full house is an intersection of hands containing three of a kind and hands that contain a pair.

## Functions

*Function* $f:A\rightarrow B$ is a rule that gives exactly one element of $B$ for every element of $A$.

$f(x)=2x+3$ is an example of $f:\mathbb{R}\rightarrow \mathbb{R}$.

$F(x,y)=0$ can define an *implicit function* of $y(x)$ or of $x(y)$.
...
*One-to-one* function: for each element of $B$ you get a distinct element of $A$.
...
## Real numbers
*Naturals* $\mathbb{N}=\{1,2,3,\ldots\}$

*Integers* $\mathbb{Z}=\mathbb{N}\cup\{0\}\cup(-\mathbb{N})$

*Rationals* $\mathbb{Q}=\{a/b: a, a\in \mathbb{Z}, b\in \mathbb{N}\}$

*Reals* $\mathbb{R}$ are different: includes $\mathbb{Q}$ and *irrationals* such as $\sqrt{2}$. Reals is a set that has a *separability* property that  $\forall X,Y\subset A \text{ such that }\forall x \in X \text{ and }\forall y \in Y x<y \Rightarrow \exists z\in A :x\leq z\leq y$.

Notice that $\mathbb{Q}$ does not have this property: $X=\{t\in\mathbb{Q}:t\leq 0 \text{ or } t^2<2\}$ and $Y=\{t\in\mathbb{Q}:t>0,\ t^2>2\}$ are not separated by any $z \in \mathbb{Q}$ (the only number between them is $\sqrt{2}$)!
...
(b) symmetric: $d(x,y)=d(y,x)$;

(c) $d(x,y)\leq d(x,z)+d(z,y)$ - triangular inequality; and
...
Set is *finite* if it is empty or there is an invertible function from the set to $\{1,2,\ldots,n\}$ for some $n\in\mathbb{N}$.

  Set is *countable* if it is finite or there is an invertible function from the set to $\mathbb{N}$.
...
A set $A$ is *open* in space *S* with metric $d(x,y)$ if $\forall x \in A \exists \varepsilon>0: B_{\varepsilon}(x) \subseteq A$.
...
Some spaces are *ordered*: there is a relation $R(x,y)$, TRUE or FALSE for each $x,y \in S$, that is transitive ($R(x,y)$ and $R(y,z)$ imply $R(x,z)$). It is a *complete* order if for any $x,y$ at least one of $R(x,y)$, $R(y,x)$ holds; otherwise it is *partial*.
...
- If you have a *utility function* over your *consumption bundles*, your preference relation is complete and ordered.
...
Prove: any singleton is convex.

Prove: $\mathbb{R}^N$ is convex.
...
\begin{scope}
\clip (-4,-2) rectangle (4,2);
 \draw[draw=red!40!white, thick] (-6,4.5)--(6,-1.5)--(6,-6)--(-6,-6);
 \end{scope}
\end{tikzpicture}
```
...
\clip (-4,-2) rectangle (4,2);
 \draw[draw=red!40!white, thick] (-6,4.5)--(6,-1.5)--(6,-6)--(-6,-6);
 \draw[draw=green!40!white, thick] (-3,5)--(5,-3)--(4,4);
 \end{scope}
\end{tikzpicture}
```
...
A function $f:D\rightarrow E$ is *continuous* at $x$ if for every $\varepsilon>0$ there is a $\delta>0$ such that for all $x'\in B_\delta(x)$ it holds that $f(x') \in B_\varepsilon (f(x))$.

Intermediate Value Theorem*: If $f(\cdot)$ is continuous on $[a,b]$, then for every value $c$ between $f(a)$ and $f(b)$ there is an $x\in[a,b]$ such that $f(x)=c$.
...
Let $f(x,y)$ be continuous function with $x$ on a compact set $X$. Then $\max_x f(x,y)$ exists and is continuous in $y$.

Extends to multiple dimensions and constraints.

*Compact* means every covering by open sets has a finite subcovering. Closed and bounded in $\mathbb{R}^N$, but not enough for $\mathbb{R}^\infty$!
...
This week:
- We introduced the concepts of sets, spaces, and functions
- We talked about how are they related to equations and systems
