...
- functions defined on $\mathbb{R}\rightarrow \mathbb{R}$
- derivatives
- integrals

## Functions

Any rule that assigns to each element of the *domain* exactly one element of the *codomain*.

- Examples: $y(x)=x^2$; metric; preference.
- Graph of $f$: the set $\{(x,y): y=f(x)\}$. Subgraph (hypograph): everything on or below the graph. Supergraph (epigraph): everything on or above the graph.
...
Inverse demand: $P(q)=Q^{-1}(q)$, the $p$ such that $Q(p)=q$ (unique if demand is strictly decreasing).
...
*Strictly increasing*: $x_1>x_2 \Rightarrow f(x_1)>f(x_2)$. *Strictly decreasing* is the opposite. *Monotone* means either increasing or decreasing.

*Weakly* increasing: $x_1>x_2 \Rightarrow f(x_1)\geq f(x_2)$.

Demand: probably decreasing? If not strictly decreasing, hard to invert.
...
```{tikz,fig.alt="Total revenue q(4-q) and total cost q^2/4 plotted against q", fig.ext='png'}
...
*Derivative* of a function at a point is the slope of the tangent line to its graph at that point.

*Integral* of a function is the *signed* area between its graph and the horizontal axis: area above the axis counts as positive, area below counts as negative.

Based on the concept of *limit*. A *sequence* $\{a_i\}$ is an ordered list $a_1,a_2,a_3,\ldots$ indexed by $i\in\{1,2,\ldots\}$ (values may repeat). 

Sequences might have limit points. $A$ is a *limit point* of $\{a_i\}$ if for every $\varepsilon>0$, $a_i\in B_\varepsilon(A)$ for infinitely many indices $i$.

Point $A$ is a *limit* of $\{a_i\}$ if for every $\varepsilon>0$, $a_i\notin B_\varepsilon(A)$ for only finitely many indices $i$.

A sequence is decreasing if $\forall i\geq 1, a_{i+1}\leq a_i$.
...
Prove: $\{1,0,1,0,1,0,\ldots\}$ does not have a limit.

Prove: $\left\{\frac{1}{1},\frac{1}{2},\frac{1}{3},\ldots\right\}$ has a limit.

Useful to know:

- $\lim (a_n+b_n)=\lim a_n+\lim b_n$ as long as all exist
...
- as $\Delta\rightarrow 0$: take every $\Delta_n \rightarrow 0,\Delta_n \not =0$
...
Prove: You know $x'=1$ and $(x^{n})'=nx^{n-1}$ for integer $n\geq 1$. Prove that $(x^{n+1})'=(n+1)x^{n}$.

Prove: $(f(x)g(x)h(x))'=f'(x)g(x)h(x)+f(x)g'(x)h(x)+f(x)g(x)h'(x)$.

Note that if you take $g(x)=1/f(x)$, you have $f(x)g(x)=1$.

Prove: $(1/f(x))'=\frac{-f'(x)}{(f(x))^2}$. Do you need an assumption on $f(x)$?
...
Prove using the chain rule: $\left(\frac{1}{f(x)}\right)'=-\frac{1}{(f(x))^2}f'(x)$
...
Imagine you are at an (interior) optimal point. What should be the derivative of the profit? Zero!

- If it is positive, produce a bit more, increase profits!
- If it is negative, produce a bit less, increase profits!

$\Pi'(q)=4-5/2q^* =0$ so $q^*=8/5.$

We will expand on optimization next week.

## Implicit Function Theorem

Suppose $f(x^*,y^*)=0$, but we can't solve $f(x,y)=0$ explicitly for $y$. ($f'_x$ and $f'_y$ denote derivatives with respect to one variable, holding the other fixed.) As long as $f'_y(x^*,y^*)\neq 0$ and the derivatives are continuous, there is a function $\phi(x)$, defined near $x^*$, such that $f(x,\phi(x))=0$ and $y^*=\phi(x^*)$.
...
At $x^*=1$, $y^*=0$ we have $f'_y=0$, and there is no such function: for $x$ slightly above 1 there is no solution, and for $x$ slightly below 1 there are two.
...
Example: if $MC(q)=a+bq$, $TC(q)=aq+\frac{b}{2} q^2+c$ --- fixed costs are impossible to deduce from MC!
...
\[u(x)=-1/x, v'(x)=1/x\]
and
\[\int \frac{\ln (x)}{x^2}dx=\ln(x)\frac{-1}{x}+C-\int \frac{-1}{x}\frac{1}{x}dx=-\frac{\ln (x)+1}{x}+C\]

## Examples

Consider a sphere of radius $R$. What is the volume of this sphere?

Answer: if we slice it by circles, each circle has an area of $\pi r^2$ for some r. What kind of $r$s do we see? Draw the sphere around $0$, take $x\in [-R,R]$. Take the set $\{(x,y,z)|x=x_0,x^2+y^2+z^2\leq R^2\}$; it's a disk of radius $r=\sqrt{R^2-x_0^2}$. Let us add these disks together as a function of x:
...
\[\left(\int_a^b f(t,x) dx\right)'_t=\int_a^b f'_t(t,x)dx.\]
...
- Derivatives: a rate of change in a function

- Integrals: a sum over values of the function

- We will be using them for optimization next week