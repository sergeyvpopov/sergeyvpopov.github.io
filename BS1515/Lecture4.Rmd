...
Weierstrass Theorem: for every continuous function $f(x)$ on an interval $[a,b]$ there exist $c,d\in [a,b]$ such that $f(c)\leq f(x) \leq f(d)$ for all $x\in[a,b]$.

So what do we do now? Imagine we have an interior solution! What does it look like?

If $f'(x)>0$, function is higher a bit to the right - we should not be able to do this if we are at the maximum.

If $f'(x)<0$, function is a bit higher to the left - we should not be able to do this if we are at the maximum.
...
$f'(x)=2x-5=0$ so is $x=5/2$ a maximum? No! Value at $x=5/2$ is $25/4-25/2+6=-6.25+6=-0.25$, but value at $x=10$ is $100-50+6=56$ so there is no maximum.
...
Take a function $f(x)=x^2+bx+c$. We want to find where its roots are: $f(x)=0$.

- Find a minimum: $x^*=-b/2$. If $f(x^*)=b^2/4-b^2/2+c=c-b^2/4>0$, the whole graph is positive $\Rightarrow$ no real roots.
...
Consider $f(x)=x^3$. Observe that $f'(0)=0$. Is that a maximum? A minimum?
 ...
 It's an *inflection* point: the function switches from concave to convex there. If $f'(0)=0$, then $f''(0)>0$ would mean a minimum and $f''(0)<0$ a maximum. But here $f''(x)=6x$, so $f''(0)=0$ and we need to look further. The first non-zero derivative at 0 is $f'''(0)=6$; because its order is odd, $x=0$ is neither a maximum nor a minimum.

## Constraints

Consider $f(x)=x^3$. Is there a maximum on $(-\infty,+\infty)$? No.

Is there a maximum on $[-2,5]$? Yes by Weierstrass theorem, and since it can't be a maximum *inside* the interval (why not?), it must be a maximum at an endpoint. $f(-2)=-8, f(5)=125$, so the maximum is at $x=5$.

Is there a maximum on $(-2,5)$?..

## Convex Analysis

 Knowing the second derivative helps identify maxima and minima. A function with $f''(x)<0$ everywhere is (strictly) *concave*: it has at most one interior maximum and no interior minima. A function with $f''(x)>0$ everywhere is (strictly) *convex*: at most one interior minimum and no interior maxima. 
 
The general definition: $f$ is convex if $f$'s supergraph is convex; e.g. $x^4$ is convex even though $f''(0)=0$, and concave if it's subgraph is convex.

Prove: Sum of two convex functions is convex.

Prove: For two convex functions of one variable, the point that minimizes their sum lies between the points that minimize each of them.

Prove: Sum of two concave functions is concave.

Prove: For two concave functions of one variable, the point that maximizes their sum lies between the points that maximize each of them.

## Multidimensional optimization

Weierstrass Theorem: for every continuous function $f(x)$ on a compact set $S$ there exist $c,d\in S$ such that $f(c)\leq f(x) \leq f(d)$ for all $x\in S$.
...
Would need to check second derivative, but now we have *three* derivatives:
...
We definitely need $f''_{xx}<0$ and $f''_{yy}<0$ for the one-dimension argument...
...
- $f''_{xx}(0,0)=-2<0$: so concave in $x$;
- $f''_{yy}(0,0)=-2<0$: so concave in $y$;
...
To establish concavity, we need a way of checking whether cross-derivatives dominate the own second derivatives...

## One way

Let's take a direction $(\alpha,\beta)\not=(0,0)$, and look what happens to
...
Observe that when $f''_{xy}=\sqrt{f''_{xx} f''_{yy}}$ (with both $f''_{xx},f''_{yy}<0$), this is equal to \[-\left(\alpha \sqrt{-f''_{xx}}-\beta \sqrt{-f''_{yy}}\right)^2\leq 0 \quad \forall (\alpha,\beta),\]
  which is exactly zero in one direction.

  Also when $f''_{xy}=-\sqrt{f''_{xx} f''_{yy}}$, this is equal to \[-\left(\alpha \sqrt{-f''_{xx}}+\beta \sqrt{-f''_{yy}}\right)^2\leq 0 \quad \forall (\alpha,\beta).\]

  So the strict inequality holds in every direction only if $(f''_{xy})^2<f''_{xx}f''_{yy}$.
...
- $f''_{xx}(x^*,y^*)>0$, $f''_{yy}(x^*,y^*)>0$ - convexity in each variable.
- $(f''_{xy}(x^*,y^*))^2<f''_{xx}(x^*,y^*) f''_{yy}(x^*,y^*)$ - cross-derivative is not dominating own derivatives from either side
...
Now observe that $f''_{ij}(x)=a_{ij}$, does not depend on $v$ or $a$; so the convexity/concavity depends on $A$ only. How to establish it? Check for positive/negative definiteness of the matrix of second derivatives!
