...
$\max_{L,S} u(L,S) \text{ s.t. } L+S\leq 8.$

Since $u'_L>0$ and $u'_S>0$, you will use all your time: the constraint binds, $L+S=8$. (The optimum may also be at a corner, $S=0$ or $S=8$, so check those too.)

$L=8-S$ so $\max_S u(8-S,S)$. Take a derivative: \[-u'_L(8-S,S)+u'_S(8-S,S)=0 \Leftrightarrow u'_L(8-S,S)=u'_S(8-S,S).\]

-   $u(L,S) = L +S^2$
-   $u(L,S) = L^{2/3} S^{1/3}$
-   $u(L,S) = \sqrt{L}+2\sqrt{S}$.

## Generic Problem: Equality Constraint
...
\[\mathcal{L}(x,y,\lambda)=u(x,y)-\lambda [f(x,y)-C]\]

choosing $x,y,\lambda$ - we are looking for max wrt $x,y,$ and $\lambda$. First order conditions are

\[\mathcal{L}'_x=0: u'_x(x,y)-\lambda f'_x(x,y)=0\]
\[\mathcal{L}'_y=0: u'_y(x,y)-\lambda f'_y(x,y)=0\]
\[\mathcal{L}'_\lambda=0: f(x,y)-C=0\]
...
\[u(C)=u(x(C),y(C))=\mathcal{L}(x(C),y(C),\lambda(C))=\]\[\quad =u(x(C),y(C))-\lambda(C) \overbrace{[f(x(C),y(C))-C]}^{=0}\]
...
Interpretation: $\lambda$, the Lagrange multiplier, is the "gain" of an extra unit of $u(\cdot,\cdot)$ if the constraint is relaxed ($C$ increases). Zero means constraint does not matter locally.

Lesson: When taking derivatives of the optimal value with respect to parameters, just take the derivative of the Lagrange function, holding the choice variables at their optimal values.
...
\[\mathcal{L}(L,K,\lambda)=rK+wL-\lambda(F(K,L)-Q).\]
...
Inequality: $\lambda$ cannot be negative! It is zero if the constraint does not bind, and can be positive only if it binds: $\lambda\,[f(x,y)-C]=0$ (*complementary slackness*).

Prove: if constraint is $f(x,y)\geq C$, $\lambda$ must be zero or negative.
...
If you have two variables and two constraints:
...
They are constrained by income $p C+m K\leq wL$, where $L$ is how much time they work.

They are constrained by time $T(L,K) \leq 24$. How to pick $C$, $K$ and $L$?
...
What would you do if you had two consumable goods, $x_1$ and $x_2$?

## Sufficient Conditions

To guarantee a maximum/minimum, need to check concavity/convexity.
...
For maximum of $f(x)$, need to check if $A$ is *negative definite*.
...
Minimum is unique if this matrix is *positive definite*.

Maximum is unique if this matrix is *negative definite*.
...
\[\mathbb{L}'_x=\frac{A}{x}-\lambda p_x=0 \Rightarrow \lambda={A}\frac{x p_x}.\]
\[\mathbb{L}'_x=\frac{B}{y}-\lambda p_y=0 \Rightarrow \lambda={B}\frac{y p_y}.\]

Therefore $\frac{A}{x p_x}=\frac{B}{y p_y}$. Substitute to the budget constraint:
\[p_x x+\frac{B}{A} p_x x = m \Rightarrow x=\frac{1}{1+B/A}\frac{m}{p_x}=\frac{A}{A+B}\frac{m}{p_x}.\]

Similarly $y=\frac{B}{A+B} \frac{m}{p_y}$.
...
## Checking the Second-Order Condition
  ...
  With 2 variables and 1 constraint, a point satisfying the first-order conditions is a local maximum if the determinant of the bordered Hessian is positive (and a local minimum if it is negative).

  Here $\det H=\frac{A}{x^2}p_y^2+\frac{B}{y^2}p_x^2>0$, so yes, we found the maximum.

  Shortcut: $u$ is concave (since $A,B>0$) and the constraint is linear, so any point satisfying the first-order conditions is a maximum.
...
- $u(x_1,x_2,x_3..x_n)=(a_1x_1^\rho+a_2 x_2^\rho+...+a_n x_n^\rho)^{1/\rho}$, $\rho<1$, $\rho\not=0$
...
- Lagrange approach to constrainted optimization
- How to check for what kind of optimum we found

Next time: In-Class Test!